...
export function retrieveTools(bot: Bot, options: ToolRetrievalOptions, cb: Callback): void
{
    ...
    const tryNextChest = () => {
        ...
        gotoChest(bot, chest, (err) => {
            ...
            pullFromChest(bot, chest, options, (gotItem: boolean, err?: Error) => {
                if (err)
                {
                    cb(err);
                    return;
...
function pullFromChest(bot: Bot, chestPos: Vec3, options: ToolRetrievalOptions, cb: (gotItem: boolean, err?: Error) => void): void
{
    const chestBlock = bot.blockAt(chestPos);
    if (!chestBlock)
    {
        cb(false, error('UnloadedChunk', 'Chest is located in an unloaded chunk!'));
        return;
    }
